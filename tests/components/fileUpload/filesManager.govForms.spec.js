define(['common/components/fileUpload/filesManager'
    , 'common/components/fileUpload/FileViewModel'
    , 'common/external/q'
    , 'common/networking/ajax'
    , 'common/core/generalAttributes'
],
    function (filesManager, File, Q, ajax, generalAttributes) {//eslint-disable-line max-params
        describe('filesManager', function () {
            let fileA = new File(), fileB = new File(), d = Q.defer();
            const mockResolver = () => {
            ...
            beforeEach(function () {
                fileA = new File();
                fileB = new File();
                spyOn(fileA, 'isRemoveActive').and.callFake(() => true);
            ...
            describe('totalAttachmentsSize', function () {
                ...
                it('should hold sum of all files sizes', function (done) {
                    const size = 1000, totalSize = 2000;
                    fileA.size(size);
                    fileB.size(size);
                    fileA.uploadFile();
                    fileB.uploadFile();
            ...
            describe('allUploadsCompleted', function () {
                ...
                it('should hold upload process for all files', function (done) {
                    ...
                    fileB.uploadFile();
                    setTimeout(function () {
                        filesManager.allUploadsCompleted().then(() => {
