...
define(['common/viewModels/ModularViewModel', 'common/components/hybridTable/hybridTable', 'common/ko/fn/config', 'common/utilities/arrayExtensions'], function (ModularViewModel, hybridTable) {
    //eslint-disable-line

    var hybridTabletExtender, Contact, extendType, typeInstance, viewModel;
    var hybridListArray = ko.observableArray();

    var matchers = {
        exceptionContains: function exceptionContains() {
            return {
                compare: function compare(actualValue, expectedResult) {
                    var res = { pass: false };
                    if (typeof actualValue === 'function') {
                        try {
                            actualValue();
                        } catch (ex) {
                            if (ex.message.indexOf(expectedResult) > -1) {
    ...
    describe('hybridTable', function () {
        ...
        beforeEach(function () {
            ...
            Contact = function Contact(settings) {
        ...
        describe('extend dynamicTable module', function () {
            ...
            it('should include texts resources', function () {
                expect(_typeof(hybridTable.texts)).toEqual('object');
            });

            it('should include events names', function () {
                expect(_typeof(hybridTable.events)).toEqual('object');
            });
        });
        ...
        describe('collapse/expand', function () {
            ...
            describe('collapse Expand all rows', function () {
                ...
                it('row inside table should be expanded', function () {
                    ...
                    var firstRow = viewModel.contactListHybrid()[0];
                ...
                it('should get table as bindingAccessor', function () {
                    ...
                    expect(function () {
                        ko.applyBindings(viewModel);
                    }).exceptionContains('table name is missing');
                });
            });
