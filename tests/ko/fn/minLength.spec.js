define(['common/ko/fn/minLength'],
function (minLength) {//eslint-disable-line no-unused-vars
    describe('fn  minLength', function () {
        ...
        it('should create the minLength as function', function () {
            expect(typeof ko.observableArray().minLength).toBe('function');
        });
        ...
        describe('types', function () {
            ...
            it('with negative number value', function () {
                var negative = -1;
                expect(function () { ko.observableArray([]).minLength(negative); }).toThrowError(error);
            });
            it('with string value', function () {
                expect(function () { ko.observableArray([]).minLength('aaa'); }).toThrowError(error);
            });
            it('with null value', function () {
                expect(function () { ko.observableArray([]).minLength(null); }).toThrowError(error);
            });
            it('with undefined value', function () {
                expect(function () { ko.observableArray([]).minLength(undefined); }).toThrowError(error);
            });
        });
    });
