define(['common/utilities/stringExtension',
 ...
 function (stringExtension, matchers, resources, exceptionMessages) //eslint-disable-line max-params
 {
     var messages = ko.multiLanguageObservable({ resource: resources });
     describe('validate', function () {

         var testArray;
         const customMessage = 'custom message invalid array';
         const customMessageObservable = ko.observable(customMessage);
         beforeEach(function () {
         ...
         describe('extensionRules Array', function () {
             describe('minItems', function () {
                 describe('setting a limit', function () {
                     ...
                     it('minRows object undefined', function () {
                         var y = undefined;
                         expect(function () { testArray.extend({ minItems: y }); }).toThrowError(stringExtension.format(exceptionMessages.funcInvalidParams, 'minItems'));
                     });
                     it('minRows null', function () {
                         expect(function () { testArray.extend({ minItems: null }); }).toThrowError(stringExtension.format(exceptionMessages.funcInvalidParams, 'minItems'));
                     });
                     it('minRows empty', function () {
                         expect(function () { testArray.extend({ minItems: '' }); }).toThrowError(stringExtension.format(exceptionMessages.funcInvalidParams, 'minItems'));
                     });
                     it('minRows not valid ', function () {
                         expect(function () { testArray.extend({ minItems: '2w' }); }).toThrowError(stringExtension.format(exceptionMessages.funcInvalidParams, 'minItems'));
                     });
             ...
             describe('maxItems', function () {
                 describe('setting a limit', function () {
                     ...
                     it('maxRows object undefined', function () {
                         var y = undefined;
                         expect(function () { testArray.extend({ maxItems: y }); }).toThrowError(stringExtension.format(exceptionMessages.funcInvalidParams, 'maxItems'));
                     });
                     it('maxRows null', function () {
                         expect(function () { testArray.extend({ maxItems: null }); }).toThrowError(stringExtension.format(exceptionMessages.funcInvalidParams, 'maxItems'));
                     });
                     it('maxRows empty', function () {
                         expect(function () { testArray.extend({ maxItems: '' }); }).toThrowError(stringExtension.format(exceptionMessages.funcInvalidParams, 'maxItems'));
                     });
                     it('maxRows not valid ', function () {
                         expect(function () { testArray.extend({ maxItems: '2w' }); }).toThrowError(stringExtension.format(exceptionMessages.funcInvalidParams, 'maxItems'));
                     });
             ...
             describe('check uniqe in one level', function () {
                 var Pet = function (name, color) {
                     this.name = name;
                     this.color = color;
                 };

                 var Owner = function (name, id) {
