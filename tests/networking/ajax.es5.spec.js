...
define(['common/networking/ajax', 'common/core/formMode'], function (ajax, formMode) {
        ...
        describe('request method', function () {
            ...
            describe('params', function () {
                ...
                it('settings object is mandatory ', function () {
                    expect(function () {
                        ajax.request().toThrow();
                    });
                });

                it('settings must be an object', function () {
                    expect(function () {
                        ajax.request(JSON.stringify(settings)).toThrow();
                    });
                });

                it('url is mandatory ', function () {
                    expect(function () {
                        ajax.request({ method: settings.method });
                    }).toThrow();
                });

                it('method is mandatory ', function () {
                    expect(function () {
                        ajax.request({ url: settings.url });
                    }).toThrow();
                });

                it('method is case sensitive ', function () {
                    expect(function () {
                        ajax.request({ url: settings.url, METHOD: settings.method });
                    }).toThrow();
                });
                ...
                it('POST method is valid ', function () {
                    expect(function () {
                        ajax.request({ url: settings.url, method: 'POST' });
                    }).not.toThrow();
                });

                it('DELETE method is invalid ', function () {
                    expect(function () {
                        ajax.request({ url: settings.url, method: 'DELETE' });
                    }).toThrow();
                });

                it('PUT method is invalid ', function () {
                    expect(function () {
                        ajax.request({ url: settings.url, method: 'PUT' });
                    }).toThrow();
                });

                it('sync request is denied ', function () {
                    expect(function () {
                        ajax.request({ url: settings.url, method: 'POST', 'async': false });
                    }).toThrowError('only async requests are supported');
                });
            ...
            describe('return a promise', function () {
                ...
                it('success handler is called when promise is resolved', function (done) {
                    ...
                    ajax.request(settings).then(function (response) {
                        result = response;
                    }, function (jqXHR) {
                        result = jqXHR;
                    }).done(function () {
                ...
                it('compose resolved promises', function (done) {
                    ...
                    ajax.request(settings).then(function (response) {
                        result = response;return ajax.request(settings);
                    }, function (jqXHR) {
                        result = jqXHR;
                ...
                it('url and method set on error object when promise is rejected', function (done) {
                    spyOn($, 'ajax').and.callFake(fakedPromiseRejector);
                    ajax.request(settings).fail(function (jqXHR) {
