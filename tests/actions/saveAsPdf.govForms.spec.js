define(['common/actions/saveAsPdf'
    ...
    function (saveAsPdf, validate, binaryAjax, languageViewModel, dialog, formInformationViewModel, fileViewer, dateMethods, userEventHandler) {//eslint-disable-line max-params
        describe('saveAsPdf', function () {
            ...
            var mutablePromise = () => { return promiseState.includes('resolve') ? fakedPromiseResolver() : fakedPromiseRejector(); };
            beforeEach(function () {
            ...
            describe('pdfRequest method', function () {
                it('without params', function () {
                    expect(function () { saveAsPdf.pdfRequest(); }).not.toThrowError();
                });
            ...
            describe('pdfSuccessCallback method', function () {
                ...
                it('when request resolved with response doesn\'t ArrayBuffer - file doesn\'t download and throw', function (done) {
                    ...
                    setTimeout(function () {
                        expect(function () { saveAsPdf.pdfReques(); }).toThrowError();
                        expect(fileViewer.downloadFileByBlob).not.toHaveBeenCalled();
                        done();
                    }, delay);
                    saveAsPdf.pdfRequest();
