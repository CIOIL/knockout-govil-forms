define(['common/core/MWResponse', 'common/components/dialog/dialog'],
    function (MWResponse, dialog) {
        describe('MWResponse', function () {
            describe('getMessageByLanguage', function () {

                it('when response is undefined throw error', function () {
                    expect(function () { MWResponse.getMessageByLanguage();}).toThrow();
                });
                it('when response.responseMessages is empty throw error', function () {
                    expect(function () { MWResponse.getMessageByLanguage({ responseMessages: '' }); }).toThrow();
                });
                it('response object include responseMessages key with string value throw error', function () {
                    expect(function () { MWResponse.getMessageByLanguage({ responseMessages: 'error' }); }).toThrow();
                });
                it('response object include responseMessages key with object value not throw', function () {
                    expect(function () { MWResponse.getMessageByLanguage({ responseMessages: { en: 'error' } }); }).not.toThrow();
                });
            });
